...
const DisplayUsers = ({
  ...
  const addUser = () => {
    ...
    if (!nextName) {
      setAddUserError("Bitte gib einen Namen ein.");
      return;
    }
    ...
    if (alreadyExists) {
      setAddUserError("Dieser Spieler existiert bereits.");
      return;
    }
  ...
  const confirmRemoveUser = (user: users) => {
    if (Platform.OS === "web") {
      const confirmed =
        typeof globalThis.confirm === "function"
          ? globalThis.confirm(`Möchtest du ${user.name} wirklich entfernen?`)
          : true;
    ...
    Alert.alert(
      "Spieler entfernen",
      `Möchtest du ${user.name} wirklich entfernen?`,
      [
        {
          text: "Abbrechen",
          style: "cancel",
  ...
  const confirmResetUserPoints = (user: users) => {
    ...
    if (Platform.OS === "web") {
      const confirmed =
        typeof globalThis.confirm === "function"
          ? globalThis.confirm(`Punkte von ${user.name} auf 0 zurücksetzen?`)
          : true;
    ...
    Alert.alert(
      "Punkte zurücksetzen",
      `Punkte von ${user.name} auf 0 zurücksetzen?`,
      [
        {
          text: "Abbrechen",
          style: "cancel",
  ...
  return (
    <View className="px-4 pb-4 w-full">
      <View className="my-3 mt-1 flex-row items-end justify-between">
        <Text className="text-2xl font-extrabold text-slate-100">Spieler</Text>
      ...
      {!users.length ? (
        <View className="mb-3 rounded-2xl border border-dashed border-slate-600 bg-slate-900/40 p-5">
          ...
          <Text className="mt-1 text-center text-slate-400">
            Füge einen Spieler hinzu, um Punkte und Vorlieben zu erfassen.
          </Text>
        </View>
      ) : null}
      {users
